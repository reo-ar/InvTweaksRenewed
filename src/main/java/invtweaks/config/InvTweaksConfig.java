...
public class InvTweaksConfig {
  ...
  public static Map<String, Category> getPlayerCats(PlayerEntity ent) {
    if (DistExecutor.callWhenOn(Dist.CLIENT, () -> () -> ent == Minecraft.getInstance().player)
        == Boolean.TRUE) {
  ...
  public static Map<String, Category> cfgToCompiledCats(List<UnmodifiableConfig> lst) {
    ...
    for (UnmodifiableConfig subCfg : lst) {
      ...
      if (!name.equals("") && !name.startsWith("/")) {
        catsMap.put(
            name,
            new InvTweaksConfig.Category(
                subCfg.getOrElse("spec", Collections.<String>emptyList())));
      }
    }
    return catsMap;
  }
  ...
  public static class ContOverride {
    ...
    @Nullable private final IntList sortRange;
